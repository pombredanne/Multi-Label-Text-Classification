...
class StructuredSelfAttentionBertNoLSTM(BasicModule):

    def __init__(self, batch_size, lstm_hid_dim, d_a, n_classes, is_train, label_input_ids, label_attention_masks):
        super(StructuredSelfAttentionBertNoLSTM, self).__init__()
        self.n_classes = n_classes

        if config.bert_model == 'bert':
            self.bert = BertModel.from_pretrained('bert-base-uncased')
            self.label_bert = BertModel.from_pretrained('bert-base-uncased')
        elif config.bert_model == 'albert':
            self.bert = AlbertModel.from_pretrained('albert-base-v1')
            self.label_bert = AlbertModel.from_pretrained('albert-base-v1')
        else:
            raise Exception('Error Bert model.')
        ...
        self.linear_first = torch.nn.Linear(config.bert_embedding_size, d_a)
        self.linear_second = torch.nn.Linear(d_a, n_classes)

        self.weight1 = torch.nn.Linear(config.bert_embedding_size, 1)
        self.weight2 = torch.nn.Linear(config.bert_embedding_size, 1)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask):
        ...
        batch_size = bert_embeddings.size(0)
        # step2 get self-attention
        selfatt = torch.tanh(self.linear_first(bert_embeddings))
        ...
        _, label_pooled_output = self.bert(input_ids=self.label_input_ids, attention_mask=self.label_attention_masks)
        # m.shape = (batch_size, n_classes, seq_length)
        m = torch.bmm(label_pooled_output.expand(batch_size, self.n_classes, config.bert_embedding_size), bert_embeddings.transpose(1, 2))

        # label_att.shape = (batch_size, n_classes, 2*lstm_hid_dim)
        label_att = torch.bmm(m, bert_embeddings)
